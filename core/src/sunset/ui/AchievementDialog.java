...
class AchievementDialog_ extends BaseDialog{
    ...
    void checkNodes(AchievementNode node){
        boolean locked = locked(node.achievement);
        if(!locked) node.visible = true;
        node.selectable = selectable(node.achievement);
        for(AchievementNode l : node.children){
            l.visible = !locked;
            checkNodes(l);
        }
    ...
    boolean selectable(SnAchievement node){
        return node.unlocked();
    }

    boolean locked(SnAchievement node){
        return node.locked();
    }
    ...
    class LayoutNode extends TreeLayout.TreeNode<LayoutNode> {
        ...
        LayoutNode(AchievementNode node, LayoutNode parent){
            this.node = node;
            this.parent = parent;
    ...
    public class AchievementNode extends TreeLayout.TreeNode<AchievementNode> {
        public final SnAchievement achievement;
        public boolean visible = true, selectable = true;

        public AchievementNode(SnAchievement achievement, AchievementNode parent){
            this.achievement = achievement;
            this.parent = parent;
            this.width = this.height = nodeSize;
            nodes.add(this);
            if(achievement.children != null){
                children = new AchievementNode[achievement.children.size];
                for(int i = 0; i < children.length; i++){
                    children[i] = new AchievementNode(achievement.children.get(i), this);
                }
            }
        }
    }
    ...
    public class View extends Group {
...

        {
            ...
            for(AchievementNode node : nodes){
                ImageButton button = new ImageButton(node.achievement.region(), Styles.nodei);
                ...
                button.clicked(() -> {
                    ...
                    }else if(canSpend(node.achievement) && locked(node.achievement)){
                        spend(node.achievement);
                    }
                });
                button.hovered(() -> {
                ...
                button.userObject = node.achievement;
                button.setSize(nodeSize);
                button.update(() -> {
                    float offset = (Core.graphics.getHeight() % 2) / 2f;
                    button.setPosition(node.x + panX + width / 2f, node.y + panY + height / 2f + offset, Align.center);
                    button.getStyle().up = !locked(node.achievement) ? Tex.buttonOver : !selectable(node.achievement) || !canSpend(node.achievement) ? Tex.buttonRed : Tex.button;

                    ((TextureRegionDrawable)button.getStyle().imageUp).setRegion(node.selectable ? node.achievement.region() : Icon.lock.getRegion());
                    button.getImage().setColor(!locked(node.achievement) ? Color.white : node.selectable ? Color.gray : Pal.gray);
        ...
        boolean canSpend(SnAchievement node){
            if(!selectable(node)) return false;
            //can always spend when locked
            return node.locked();
        }

        void spend(SnAchievement node){
...
//                    shine[i] = true;
        ...
        void unlock(SnAchievement node) {
            node.unlock();

            //unlock parent nodes in multiplayer.
            SnAchievement parent = node.parent;
            while (parent != null) {
                parent.unlock();
            ...
            Core.scene.act();
            Sounds.unlock.play();
//            Events.fire(new EventType.ResearchEvent(node));
        }
        ...
        void rebuild(){
            ...
            SnAchievement node = (SnAchievement)button.userObject;
            ...
            infoTable.table(b -> {
                ...
                if(selectable && (node.description() != null || node.hasMoveInfo())){
                    b.button(Icon.info, Styles.cleari, () -> ui.content.show(node.content)).growY().width(50f);
                }
                b.add().grow();
                b.table(desc -> {
                    desc.left().defaults().left();
                    desc.add(selectable ? node.localizedName() : "[accent]???");
                    desc.row();
                    if(locked(node)){
        ...
        public void drawChildren(){
            ...
            for(AchievementNode node : nodes){
                ...
                for(AchievementNode child : node.children){
                    ...
                    boolean lock = locked(node.achievement) || locked(child.achievement);
                    Draw.z(lock ? 1f : 2f);
