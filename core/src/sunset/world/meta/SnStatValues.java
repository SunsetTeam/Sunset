...
//                t.add(Core.bundle.format("stat.drillitemdesc", count, di.item.emoji(), di.item.localizedName));
    ...
    public static <T extends UnlockableContent> StatValue ammo(ObjectMap<T, BulletType> map, int indent){
        if (!map.values().toSeq().contains(b->b== SnBullets.blockBullet))return StatValues.ammo(map,indent);
        return table -> {
            ...
            for(T t : orderedKeys){
                ...
                table.table(bt -> {
                    ...
                    if (!(type instanceof BlockBulletType blockBulletType && blockBulletType.displayStats(t,bt))){
                        if(type.damage > 0 && (type.collides || type.splashDamage <= 0)){
                            if(type.continuousDamage() > 0){
                                bt.add(Core.bundle.format("bullet.damage", type.continuousDamage()) + StatUnit.perSecond.localized());
                            }else{
