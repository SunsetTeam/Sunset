...
import sunset.type.planets.SnSector;
import sunset.world.blocks.environment.MockEnvironmentBlock;

import static mindustry.Vars.*;

public class SnPlanetGenerator extends PlanetGenerator{
    //alternate, less direct generation (wip)
    public static boolean alt = false;
    private static int totalSeed = 0;
    public final int seed = totalSeed++;
    ...
    public void genTile(Vec3 position, TileGen tile){
        Block block = getBlock(position);
        if(block instanceof MockEnvironmentBlock env){
            block = env.replacement;
        }
        tile.floor = block;
    ...
    protected final void generate(){
        ...
        //rivers
        pass((x, y) -> {
            passRivers(fspawn, x, y);
        });

        Seq<Block> ores = Seq.with(Blocks.oreCopper, Blocks.oreLead);
        float poles = Math.abs(sector.tile.v.y);

        setupOres(ores, poles, 0.5f, 1f, 1.3f);
        ...
        SnSector sector = snSector();
        if (sector!=null){
            state.rules.spawns = sector.waves().generate(difficulty, new Rand(), state.rules.attackMode);
        }
//        state.rules.spawns = SunsetWaves.generate(difficulty, new Rand(sector.id), state.rules.attackMode, state.rules.attackMode && SunsetVars.spawner.countGroundSpawns() == 0);
    ...
    private SnSector snSector(){
        return sector instanceof SnSector? (SnSector)sector :null;
    }
    ...
    protected void passTiles(Seq<Room> roomseq, int x, int y){
        //hotrock tweaks
    ...
    protected void randomStaffPass(int x, int y){
        for(int i = 0; i < 4; i++){
