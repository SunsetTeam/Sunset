...
import sunset.utils.V7.arc.interfaces.HexMesher;
...
public class ModGenerator extends PlanetGenerator{
    ...
    public Block getBlock(Vec3 pos) {
        ...
        float tnoise = (float) Simplex.noise3d(0, 7d, 0.56d, 1d / 3d, pos.x, pos.y + 999d, pos.z);
        temp = Mathf.lerp(temp, tnoise, 0.5f);
        height *= 0.9f;
        height = Mathf.clamp(height);

        float tar = (float) Simplex.noise3d(0, 4, 0.55, 0.5, pos.x, pos.y + 999, pos.z) * 0.3f + Tmp.v31.dst(0, 0, 1) * 0.2f;
        Block res = arr[
    ...
    public float rawHeight(Vec3 pos) {
        ...
        return (Mathf.pow((float) Simplex.noise3d(0, 7, 0.5, 1d / 3d, pos.x, pos.y, pos.z), 2.3f) + waterOffset) / (1 + waterOffset);
    }
