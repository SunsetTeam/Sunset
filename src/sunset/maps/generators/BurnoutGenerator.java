...
public class BurnoutGenerator extends PlanetGenerator{
    ...
    protected void generate(){
        ...
        pass((x, y) -> {
            ...
            }else if(genLakes && floor != Blocks.basalt && floor != Blocks.slag && floor.asFloor().hasSurface()){
                float noise = noise(x + 782, y, 5, 0.75f, 260f, 1f);
                if(noise > 0.67f && !roomseq.contains(e -> Mathf.within(x, y, e.x, e.y, 14))){
                    if(noise > 0.72f){
                        floor = noise > 0.78f ? Blocks.slag : (floor == SnEnvironment.ash ? SnEnvironment.burningash : SnEnvironment.obsidian);
                    }else{
                        floor = (floor == Blocks.darksand ? floor : SnEnvironment.ash);
                    }
                }
            }
